...
def main():
    ...
    trainset = SiameseHyperFeat(HyperFeat(train_data_path, trainval='trainval' if args.combine_trainval else 'train',
                                          L=args.L, window=args.window))
    testset = SiameseHyperFeat(HyperFeat(test_data_path, trainval='val', L=args.L, window='Inf', ))
    ...
    model_dict = checkpoint['state_dict']
    metric_net.module.load_state_dict(model_dict)
    test(args, metric_net, test_loader, criterion, save_result=args.save_result, epoch=args.epochs)
