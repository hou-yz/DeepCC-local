...
def test(args, model, test_loader, criterion):
    ...
    miss = 0
    with torch.no_grad():
        for epoch in range(1, test_loader.dataset.num_spatialGroup + 1):
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        test_loss, correct, correct + miss, 100. * correct / (correct + miss)))
...
def main():
    ...
    test_loader = DataLoader(dataset, batch_size=args.test_batch_size,
                             sampler=RandomIdentitySampler(dataset, 100),
                             num_workers=0, pin_memory=True)
