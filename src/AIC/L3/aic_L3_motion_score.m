...
% 
% short_time       = intervals(:,1)' - intervals(:,2);
% short_distance_x = start_points(:,1)' - end_points(:,1);
% short_distance_y = start_points(:,2)' - end_points(:,2);
% short_distance = sqrt(short_distance_x.^2 + short_distance_y.^2);
% short_time(short_time<=0) = 0;  short_distance(short_time<=0) = 0; 
% % impossibility((short_distance - 10)./(short_time+10^-12) * 10 > opts.identities.speed_limit(1)) = 1;
% % impossibility((short_distance + 10)./(short_time+10^-12) * 10 < opts.identities.speed_limit(2)) = 1;
