...
for i = 1 : length(currentIdentities)
    for k = 1 : length(currentIdentities(i).trajectories) 
        ...
        if currentIdentities(i).trajectories(k).startFrame == currentIdentities(i).startFrame
            start_indicator(length(trajectoryLabels)) = true;
        else
            start_indicator(length(trajectoryLabels)) = false;
        end
        if currentIdentities(i).trajectories(k).endFrame == currentIdentities(i).endFrame
            end_indicator(length(trajectoryLabels)) = true;
        else
            end_indicator(length(trajectoryLabels)) = false;
        end
...
result = solveInGroupsIdentities_aic(opts, trajectories(inAssociation), trajectoryLabels(inAssociation),appear_model_param,motion_model_param,start_indicator(inAssociation),end_indicator(inAssociation));
