...
%     detections          = load(sprintf('%s/train/S%02d/c%03d/det/det_yolo3.txt', opts.dataset_path, opts.scene_by_icam(iCam), iCam));
    detections          = load(sprintf('%s/train/S%02d/c%03d/gt/gt.txt', opts.dataset_path, opts.scene_by_icam(iCam), iCam));
...
%         fig = imread(sprintf('%s/ALL_det_bbox/val/s%02d_c%02d_f%05d_%03d.jpg', opts.dataset_path, opts.scene_by_icam(iCam), iCam, detections(j, 1), counter));
        fig = imread(sprintf('%s/ALL_gt_bbox/val/gt_bbox_10_fps/%04d_s%02d_c%02d_f%05d.jpg', opts.dataset_path, detections(j, 2), opts.scene_by_icam(iCam), iCam, detections(j, 1)));
