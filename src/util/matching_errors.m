function [gtTP, predTP] = matching_errors( tr1, tr2, THRESHOLD, world )
...
if ~overlapTest
    ...
    fn = length(frames_gt);
    cost = fp + fn;
    gtTP = zeros(size(tr1,1),1);
    predTP = zeros(size(tr2,1),1);
    return;
end
...
% Detections too far
gtTooFar = find(distanceGTvsPred < THRESHOLD);
predTooFar = find(distancePredvsGT < THRESHOLD);
indsGTFound = find(isfoundGT);
indsPredFound = find(isfoundPred);

gtTP(indsGTFound(gtTooFar)) = 0;
predTP(indsPredFound(predTooFar)) = 0;
