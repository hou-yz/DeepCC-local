% Computes the types of error (IDFP, IDFN) to be used for visualization or
% diagnorsing the tracker result
...
function [gtMat2, predMat2] = error_types( groundTruthMat, predictionMat, threshold, world )
% Input: 
...
gtTP = cell(length(idsGT),1);
predTP = cell(length(idsPred),1);
sumTP = 0;
for i = 1:length(idsGT)
    if assignment(i) <= length(predTP)
        [gtTP{i}, predTP{assignment(i)}] =  matching_errors(ground_truth{i},prediction{assignment(i)}, threshold, world);
        ground_truth{i}(:,end) = gtTP{i};
        prediction{assignment(i)}(:,end) = predTP{assignment(i)};
        
        idrec(i) = sum(gtTP{i}) / size(ground_truth{i},1);
        sumTP = sumTP + sum(gtTP{i});
    end
end 

gtMat2 = [];
for k = 1:length(ground_truth)
    gtMat2 = [gtMat2; ground_truth{k}];
end
predMat2 = [];
for k = 1:length(prediction)
    predMat2 = [predMat2; prediction{k}];
end
