function [allMets, metsBenchmark, metsMultiCam] = evaluateTracking(seqmap, resDir, gtDataDir, benchmark, dataset_path)
...
for ind = 1:8
    ...
    if ~multicam
        ...
        if strcmp(seqmap,'DukeMTMCT-test-easy.txt')
            load(fullfile(dataset_path,'ground_truth', 'testData.mat'));
            gtdata = testData;
            testInterval = [263504:356648];
        elseif strcmp(seqmap,'DukeMTMCT-test-hard.txt')
            load(fullfile(dataset_path,'ground_truth', 'testHardData.mat'));
            gtdata = testHardData;
